            ...
            <div class="card-body">
                <table>
                    <?php
                        ...
                        $id_attempt     = $_GET['id_attempt_guest'];
                        $limit          = (isset($_GET['limit']) && $_GET['limit'] >= 1) ? $_GET['limit'] : 1;
                        $hasil          = mysqli_query($koneksi, "SELECT * FROM tbl_soal INNER JOIN tbl_attempt_guest ON tbl_soal.id_modul = tbl_attempt_guest.id_modul INNER JOIN tbl_jwb_guest ON tbl_soal.id_soal = tbl_jwb_guest.id_soal WHERE tbl_soal.id_modul = '$id_modul' AND tbl_attempt_guest.id_attempt_guest = '$id_attempt'");
                        ...
                        $tampil         = mysqli_query($koneksi, "SELECT * FROM tbl_soal INNER JOIN tbl_attempt_guest ON tbl_soal.id_modul = tbl_attempt_guest.id_modul INNER JOIN tbl_jwb_guest ON tbl_soal.id_soal = tbl_jwb_guest.id_soal WHERE tbl_soal.id_modul = '$id_modul' AND tbl_attempt_guest.id_attempt_guest = '$id_attempt' LIMIT $dataAwal, $limit ");
                        ...
                        <nav aria-label="Page navigation example">
                            <ul class="pagination">
                                <?php for ( $k = 1; $k <= $totalHalaman; $k++ ) : ?>
                                    <?php if ($k == $halaman) : ?>
                                        <a type="submit" href="?halaman=modul_praktik&perihal=preview_guest&id_attempt_guest=<?= $id_attempt ?>&id_modul=<?= $id_modul ?>&page=<?= $k; ?>" class="btn btn-light active"><?= $k; ?></a>
                                    <?php else : ?>
                                        <a type="submit" href="?halaman=modul_praktik&perihal=preview_guest&id_attempt_guest=<?= $id_attempt ?>&id_modul=<?= $id_modul ?>&page=<?= $k; ?>" class="btn btn-light"><?= $k; ?></a>
                                    <?php endif; ?>
                                <?php endfor; ?>
                            </ul>
                        </nav>
                ...
                <div class="mt-2 justify-content-end d-flex">
                    <nav aria-label="Page navigation example">
                        <ul class="pagination">
                            <?php if ($halaman > 1) : ?>
                                <a type="submit" href="?halaman=modul_praktik&perihal=preview_guest&id_attempt_guest=<?= $id_attempt ?>&id_modul=<?= $id_modul ?>&page=<?= $halaman - 1 ?>" class="btn btn-danger" name="previous">Previous</a>
                            <?php endif; ?>
                            <?php if ($halaman < $totalHalaman) : ?>
                                <a type="submit" href="?halaman=modul_praktik&perihal=preview_guest&id_attempt_guest=<?= $id_attempt ?>&id_modul=<?= $id_modul ?>&page=<?= $halaman + 1 ?>" class="btn btn-primary" name="next">Next</a>
                            <?php endif; ?>
                        </ul>
                    </nav>
                </div>
