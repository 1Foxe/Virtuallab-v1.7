...
if (isset($_POST['bSimpan'])) {
    ...
    $id_lab = $_SESSION["id_laboran"];
    $matkul = $_POST['mata_kuliah'];

    //Cek dulu apakah mata kuliah ini sudah di daftarkan sebelumnya
    $cekMatkul = mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM tbl_labpengampu WHERE id_matkul = '$matkul' AND id_laboran = '$id_lab'"));
    ...
    if($cekMatkul > 0) {
        ...
        mysqli_query($koneksi, "INSERT INTO tbl_labpengampu(id_laboran, id_matkul) VALUES ('$id_lab', '$matkul') ");
        echo "<script src='./assets/js/sweetalert2/dist/sweetalert2.min.js'></script>
        <link rel='stylesheet' href='./assets/js/sweetalert2/dist/sweetalert2.min.css'>";
        echo "<script type='text/javascript'>
            setTimeout(function() {
...
<div class="row">
    <?php
    $id_lab = $_SESSION["id_laboran"];
    $tampil = mysqli_query($koneksi, "SELECT tbl_matakuliah.*, tbl_labpengampu.* FROM tbl_matakuliah, tbl_labpengampu WHERE tbl_matakuliah.id_matkul = tbl_labpengampu.id_matkul AND id_laboran = '$id_lab' ");
    $no = 1;
    while ($data = mysqli_fetch_array($tampil)) :
    ?>
            ...
            <div class="card border-left-primary shadow h-100 py-2">
                <div class="card-body">
                    <div class="row no-gutters align-items-center">
                        ...
                        <div class="col-auto">
                            <a href="?halaman=pilih_matkul&perihal=hapus&id=<?=$data['id_matkul']?>&id_lab=<?=$id_lab?>" class="delete-enroll" data-toggle="tooltip" title="Unenroll?" data-placement="top"><i class="fa-solid fa-square-minus fa-2x "></i></a>
