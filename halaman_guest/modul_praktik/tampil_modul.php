...
<div class="row justify-content-center">
    ...
    $id_modul       = $_GET['id_modul'];
    $id_matkulguest = $_GET['id_matkulguest'];
    $tampil         = mysqli_query($koneksi, "SELECT * FROM tbl_modulpraktik INNER JOIN tbl_matkulguest ON tbl_modulpraktik.id_matkul = tbl_matkulguest.id_matkul WHERE tbl_matkulguest.id_matkulguest = '$id_matkulguest' AND tbl_modulpraktik.id_modul = '$id_modul'");
    $data           = mysqli_fetch_array($tampil);
...
<?php if ($read['id_soal']) : ?>
    <div class="row justify-content-center mt-2 mb-2">
        <div class="col-sm-2">
            <div class="card">
                <a href="?halaman=modul_praktik&perihal=attempt_quiz&id_matkulguest=<?= $data['id_matkulguest'] ?>&id_modul=<?= $data['id_modul'] ?>" target="_blank" class="btn btn-light btn-sm">Quiz</a>
            </div>
        </div>
    </div>
<?php endif; ?>
