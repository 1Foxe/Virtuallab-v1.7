            ...
            <div class="card-body">
                        ...
                        <nav aria-label="Page navigation example">
                            <ul class="pagination">
                                <?php for ( $k = 1; $k <= $totalHalaman; $k++ ) : ?>
                                    <?php if ($k == $halaman) : ?>
                                        <a type="submit" href="?halaman=modul_praktik&perihal=preview_quiz&id_attempt_guest=<?= $id_attempt ?>&id_modul=<?= $id_modul ?>&page=<?= $k; ?>" class="btn btn-light active"><?= $k; ?></a>
                                    <?php else : ?>
                                        <a type="submit" href="?halaman=modul_praktik&perihal=preview_quiz&id_attempt_guest=<?= $id_attempt ?>&id_modul=<?= $id_modul ?>&page=<?= $k; ?>" class="btn btn-light"><?= $k; ?></a>
                                    <?php endif; ?>
                                <?php endfor; ?>
                            </ul>
                        </nav>
                ...
                <div class="mt-2 justify-content-end d-flex">
                    <nav aria-label="Page navigation example">
                        <ul class="pagination">
                            <?php if ($halaman > 1) : ?>
                                <a type="submit" href="?halaman=modul_praktik&perihal=preview_quiz&id_attempt_guest=<?= $id_attempt ?>&id_modul=<?= $id_modul ?>&page=<?= $halaman - 1 ?>" class="btn btn-danger" name="previous">Previous</a>
                            <?php endif; ?>
                            <?php if ($halaman < $totalHalaman) : ?>
                                <a type="submit" href="?halaman=modul_praktik&perihal=preview_quiz&id_attempt_guest=<?= $id_attempt ?>&id_modul=<?= $id_modul ?>&page=<?= $halaman + 1 ?>" class="btn btn-primary" name="next">Next</a>
                            <?php endif; ?>
                        </ul>
                    </nav>
                </div>
