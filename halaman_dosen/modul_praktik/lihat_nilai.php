...
<button type="button" class="btn btn-primary" id="checkQuiz">Check Quiz</button>
<!-- <a href="?halaman=modul_praktik&perihal=check_quiz&id_matkul=<?= $id_matkul ?>&kode_matkul=<?= $data['kode_matkul'] ?>&nama_matkul=<?= $data['nama_matkul'] ?>" class="btn btn-primary btn-lx mb-4">
    <i class="fa-solid fa-check-double mr-2"></i>Check Enroll
</a> -->
<a href="?halaman=modul_praktik&perihal=exportnilai&id_modul=<?= $data['id_modul']?>" target="_blank" class="btn btn-info">Export Nilai</a>
<div id="tbl_mhs">
...
<script>
    ...
    $(document).ready(function(){
        ...
        $('#checkQuiz').click(function() {
        var id_matkul = <?= $data['id_matkul'] ?>;
        var kode_matkul = '<?= $data['kode_matkul'] ?>';
        var nama_matkul = '<?= $data['nama_matkul'] ?>';

        window.location.href = "?halaman=modul_praktik&perihal=check_quiz&id_matkul=" + id_matkul + "&kode_matkul=" + kode_matkul + "&nama_matkul=" + nama_matkul;
    });
