...
<?php
    ...
    $query = mysqli_query($koneksi, "SELECT * FROM tbl_modulpraktik INNER JOIN tbl_matkulmhs ON tbl_modulpraktik.id_matkul = tbl_matkulmhs.id_matkul WHERE tbl_matkulmhs.id_matkulmhs = '$id_matkulmhs' AND tbl_modulpraktik.id_modul = '$id_modul'");
    ...
    if(!isset($_GET['id_attempt']) && !isset($_POST['id_attempt'])){
        $query2 = mysqli_query($koneksi, "INSERT INTO tbl_attempt_mhs (id_matkulmhs, id_modul, waktu, tanggal_kerja) VALUES ('$id_matkulmhs', '$id_modul', '$waktu', CURRENT_TIMESTAMP() )");
            ...
            <div class="card-body">
                <div class=" justify-content-end d-flex" id="timeRR"></div>
                    <?php
                        ...
                        $hasil          = mysqli_query($koneksi, "SELECT * FROM tbl_soal INNER JOIN tbl_attempt_mhs ON tbl_soal.id_modul = tbl_attempt_mhs.id_modul WHERE tbl_attempt_mhs.id_attempt_mhs = '$id_attempt' AND tbl_soal.id_modul = '$id_modul'");
                        ...
                        $next           = $halaman + 1;
                        $previous       = $halaman - 1;
                        $tampil         = mysqli_query($koneksi, "SELECT * FROM tbl_soal INNER JOIN tbl_attempt_mhs ON tbl_soal.id_modul = tbl_attempt_mhs.id_modul WHERE tbl_attempt_mhs.id_attempt_mhs = '$id_attempt' AND tbl_soal.id_modul = '$id_modul' LIMIT $dataAwal, $limit");
                    ?>
                    ...
                    <?php
                        ...
                        $cekJawab       = mysqli_query($koneksi, "SELECT id_jwb_mhs, jawaban, jawab_essay FROM tbl_jwb_mhs WHERE id_attempt_mhs = '$id_attempt' AND id_soal = '$id_soal'");
                        if (mysqli_num_rows($cekJawab) == 1) {
                            $hasilJawab     = mysqli_fetch_array($cekJawab);
                            $id_jwb_mhs     = $hasilJawab['id_jwb_mhs'];
                    ...
                    ?>
                        ...
                        <input type="hidden" name="id_jwb_mhs" value="<?= $id_jwb_mhs ?? NULL; ?>">
                        <input type="hidden" name="id_matkulmhs" value="<?= $id_matkulmhs ?? NULL; ?>">
                    ...
                    <?php
                        if ( $_SERVER['REQUEST_METHOD'] == 'POST') {
                            ...
                            if ( in_array($id_jwb_mhs, [NULL, ''] ) ) {
                                $input      = mysqli_query($koneksi, "INSERT INTO tbl_jwb_mhs (id_attempt_mhs, id_soal, jawaban, jawab_essay) VALUE ('$id_attempt', '$id_soal', '$jawab', '$jawab_essay')");
                            } else {
                                $input      = mysqli_query($koneksi, "UPDATE tbl_jwb_mhs SET jawaban = '$jawab', jawab_essay = '$jawab_essay' WHERE id_attempt_mhs = '$id_attempt' AND id_soal = '$id_soal'");
                            }
                            $query          = mysqli_query($koneksi, "SELECT * FROM tbl_jwb_mhs INNER JOIN tbl_attempt_mhs ON tbl_jwb_mhs.id_attempt_mhs = tbl_attempt_mhs.id_attempt_mhs WHERE tbl_jwb_mhs.id_attempt_mhs = '$id_attempt'");
                            if (isset($_POST['previous'])) {
                                echo "<script>
                                    document.location='?halaman=modul_praktik&perihal=quiz&id_matkulmhs=$id_matkulmhs&id_modul=$id_modul&id_attempt=$id_attempt&page=$previous';
                                </script>";
                            } elseif (isset($_POST['next'])) {
                                echo "<script>
                                    document.location='?halaman=modul_praktik&perihal=quiz&id_matkulmhs=$id_matkulmhs&id_modul=$id_modul&id_attempt=$id_attempt&page=$next';
                                </script>";
                            } elseif (isset($_POST['submitQuiz'])) {
                                $benar = 0;
                                $salah = 0;
                                $total = 0;
                                $query = mysqli_query($koneksi, "SELECT * FROM tbl_soal WHERE id_modul = '$id_modul'");
                                ...
                                if ($queryNilai) { 
                                    ...
                                    window.setTimeout(function(){
                                        document.location='?halaman=modul_praktik&perihal=attempt_quiz&id_matkulmhs=$id_matkulmhs&id_modul=$id_modul';
                                    }, 2000);
                                    </script>";
                                }
                                ...
                                echo "<script>
                                    document.location='?halaman=modul_praktik&perihal=quiz&id_matkulmhs=$id_matkulmhs&id_modul=$id_modul&id_attempt=$id_attempt&page=$k';
                                </script>";
                            }
                        }
                    ?>
    ...
    const timer = () => {
        ...
        return () => {
            ...
            if (sisa == 0) {
                $.ajax({
                    type: 'POST',
                    url: 'submit_quiz_mhs.php',
                    data: { 
                        simpanQuiz: '',
                        id_matkulmhs: '<?=$data['id_matkulmhs']?>',
                    ...
                    success: function(response) {
                        ...
                        window.setTimeout(function(){
                            document.location='?halaman=modul_praktik&perihal=attempt_quiz&id_matkulmhs=<?= $data['id_matkulmhs'] ?>&id_modul=<?= $data['id_modul'] ?>';
                        }, 1000);
                    }
                });
            }
